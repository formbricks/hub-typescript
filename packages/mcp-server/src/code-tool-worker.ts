...
function getTSDiagnostics(code: string): string[] {
  ...
  const codeWithImport = [
    ...
    functionSource.type === 'declaration' ?
      `async function run(${functionSource.client}: FormbricksHub)`
    : `const run: (${functionSource.client}: FormbricksHub) => Promise<unknown> =`,
    functionSource.code,
  ].join('\n');
...
const fuse = new Fuse(
  [
    ...
    'client.feedbackRecords.retrieve',
    'client.feedbackRecords.retrieveSimilar',
    'client.feedbackRecords.update',
    'client.feedbackRecords.search.performSemanticSearch',
