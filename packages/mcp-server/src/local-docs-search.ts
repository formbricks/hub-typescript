...
const EMBEDDED_METHODS: MethodEntry[] = [
  ...
  },
  {
    ...
    params: ['tenant_id: string;'],
    response: '{ settings: { sentiment_enabled?: boolean; target_language?: string; }; tenant_id: string; }',
    markdown:
      "## retrieve\n\n`client.tenants.settings.retrieve(tenant_id: string): { settings: object; tenant_id: string; }`\n\n**get** `/v1/tenants/{tenant_id}/settings`\n\nReturns the enrichment settings for the specified tenant_id. A tenant that has not configured any\nsettings yet returns HTTP 200 with default (unset) values rather than 404; consumers decide the\nfallback behavior for unset values. Settings are tenant-scoped and never shared across tenants.\n\n\n### Parameters\n\n- `tenant_id: string`\n\n### Returns\n\n- `{ settings: { sentiment_enabled?: boolean; target_language?: string; }; tenant_id: string; }`\n\n  - `settings: { sentiment_enabled?: boolean; target_language?: string; }`\n  - `tenant_id: string`\n\n### Example\n\n```typescript\nimport FormbricksHub from '@formbricks/hub';\n\nconst client = new FormbricksHub();\n\nconst setting = await client.tenants.settings.retrieve('org-123');\n\nconsole.log(setting);\n```",
    perLanguage: {
      typescript: {
