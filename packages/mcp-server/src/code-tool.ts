...
const remoteStainlessHandler = async (
  ...
  const localClientEnvs = { HUB_API_KEY: requireValue(readEnv('HUB_API_KEY') ?? client.apiKey, 'set HUB_API_KEY environment variable or provide apiKey client option'), FORMBRICKS_HUB_BASE_URL: readEnv('FORMBRICKS_HUB_BASE_URL') ?? client.baseURL ?? undefined };
...
const localDenoHandler = async (
  ...
  try {
    const sdkPkgName = '@formbricks/hub';
